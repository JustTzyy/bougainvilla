...
<script>
  (function(){
    ...
    function applySearch(){
      ...
      if (!q) { 
        ...
        filteredRows = allRows.filter(function(r){
          var t = (''+r.user_name+' '+r.room+' '+r.accommodation+' '+r.check_in+' '+r.status+' '+r.amount+' '+r.date).toLowerCase();
    ...
    function renderTable(){
      ...
      alignAllArchivedTransactionsTable();
    }
    ...
    function alignAllArchivedTransactionsTable(){
      try {
        var tbody = document.getElementById('allTransactionsTable').getElementsByTagName('tbody')[0];
        Array.prototype.forEach.call(tbody.rows, function(row){
          ...
          // Color status text (Status is column index 4)
