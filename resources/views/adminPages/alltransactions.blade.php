  ...
  (function(){
    ...
    allRows = rows.map(function(row) {
      ...
      return {
        ...
        amount: cells[6] ? cells[6].textContent.trim() : '',
        date: cells[7] ? cells[7].textContent.trim() : '',
        element: row
      };
    });
    ...
    function applySearch(){
      ...
      if (!q) { 
        ...
        filteredRows = allRows.filter(function(r){
          var t = (''+r.user_name+' '+r.room+' '+r.accommodation+' '+r.check_in+' '+r.check_out+' '+r.status+' '+r.amount+' '+r.date).toLowerCase();
    ...
    function alignAllTransactionsTable(){
      try {
        var tbody = document.getElementById('allTransactionsTable').getElementsByTagName('tbody')[0];
        Array.prototype.forEach.call(tbody.rows, function(row){
          ...
          // Color status text (Status is column index 5)
    ...
    function displayGuestDetails(guests, transaction) {
      ...
      // Cleaner details section (if provided by API)
